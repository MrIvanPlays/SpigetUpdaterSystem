...
/** Represents parent class for the updaters, giving a bunch of abilities to the updater classes */
public abstract class Updater {
  ...
  public Updater(int resourceID, String currentVersion, String pluginName) {
    ...
    this.VERSION_URL =
        "https://api.spiget.org/v2/resources/" + resourceID + "/versions?size=" + Integer.MAX_VALUE;
    this.DESCRIPTION_URL =
        "https://api.spiget.org/v2/resources/" + resourceID + "/updates?size=" + Integer.MAX_VALUE;
  ...
  /**
   * Fetchs the latest version and update title from spiget into a String array. [0] - version [1] -
   * update title
   *
   * @return string array with the current version and update title on spigot, fetched via spiget
   */
  protected String[] getLastUpdate() {
    try {
      ...
      JsonArray versionsArray =
          parser
              .parse(IOUtils.toString(new URL(VERSION_URL), Charset.defaultCharset()))
              .getAsJsonArray();
      String lastVersion =
          versionsArray.get(versionsArray.size() - 1).getAsJsonObject().get("name").getAsString();
      if (!lastVersion.equalsIgnoreCase(currentVersion)) {
        JsonArray updatesArray =
            parser
                .parse(IOUtils.toString(new URL(DESCRIPTION_URL), Charset.defaultCharset()))
                .getAsJsonArray();
        String updateName =
            updatesArray.get(updatesArray.size() - 1).getAsJsonObject().get("title").getAsString();
  ...
  /**
   * Desired update message, which is send to the player. Created via component api.
   *
  ...
  protected BaseComponent[] message() {
    ComponentBuilder builder =
        new ComponentBuilder("[")
            ...
            .color(ChatColor.GOLD)
            .append(" ")
            .append("New update available")
            .color(ChatColor.GREEN)
            .append("\n")
            .append("You are currently running version " + currentVersion)
            .color(ChatColor.RED)
            .append("\n")
            .append("New version: " + getLastUpdate()[0])
            .color(ChatColor.GREEN)
            .append("\n")
            .append("What's new: \"" + getLastUpdate()[1] + "\"")
            .color(ChatColor.AQUA)
            .append("\n")
            .append("You can download via clicking ")
    ...
    linkComponent.setHoverEvent(
        new HoverEvent(
            HoverEvent.Action.SHOW_TEXT,
            new ComponentBuilder("Click me to get redirect to download page")
  ...
  protected void updateMessageConsole(Logger logger) {
    logger.warning(
        "Stable version "
            + getLastUpdate()[0]
            + " is out! You are still running version "
            + currentVersion);
